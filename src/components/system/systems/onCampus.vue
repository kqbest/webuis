  ...
  <section class="onCampus">
    ...
    <section class="this_table">
      <el-table :data="tableData" border style="width: 100%">
        ...
        <el-table-column prop="stateName" align="center">
          <template scope="scope">
            <span class="this_enable" @click="dialogVisible = true;enabledState(scope.row.menuID,scope.row.state)">{{ scope.row.stateName }}</span>
          </template>
        </el-table-column>
      </el-table>
    </section>
...
var title = 'http://192.168.0.48:6001/';
var local = {
  findTree: title + 'systemmanagement/systemsetting/sysMenuManager/findTree',
  findPage: title + 'systemmanagement/systemsetting/sysMenuManager/findPage',
  updateState: title + 'systemmanagement/systemsetting/sysMenuManager/updateState'
}
export default {
  data() {
    return {
  ...
  methods: {
    //加载树形
    findTree() {
      axios.get(local.findTree).then(data => {//menuID menuName
